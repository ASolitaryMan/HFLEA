...
class HuBERTPreTrainModule(LightningModule):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        # config = AutoConfig.from_pretrained(hubert_path)
        self.model = HubertModelPreTrainBase.from_pretrained(hubert_path, num_cluster=num_classes, feature_grad_mult=feature_grad_mult)
        self.model.config.mask_time_prob = 0.8
    ...
    def _step(self, batch: Batch, batch_idx, step_type):
        ...
        attention_mask = torch.ones(waveforms.size(), dtype=torch.int16, device=waveforms.device)
        #传attention_mask
        if step_type == "val":
            with torch.no_grad():
                outputs = self.model(
                ...
                logit_m = outputs.logit_m
                logit_u = outputs.logit_u
                feature_penalty = outputs.features_pen
        else:
            outputs = self.model(
                    input_values = waveforms,
                    attention_mask = attention_mask,
        ...
        self.log(
            ...
            # self.mask_stats[step_type]["correct"],
            # self.mask_stats[step_type]["count"],
            on_step=True,
            on_epoch=True,
            sync_dist=True,
        ...
        self.log(
            ...
            # self.unmask_stats[step_type]["correct"],
            # self.unmask_stats[step_type]["count"],
            on_step=True,
            on_epoch=True,
            sync_dist=True,
            prog_bar=step_type == "train",
    ...
    def training_step(self, batch: Batch, batch_idx):
        ...
        self.log("Gathered_number_of_frames", num_frames.float().sum(), on_step=True, on_epoch=True)
