...
class CollateFnHubert:
    ...
    def __call__(self, batch: List[Tuple[Tensor, Tensor, int]]) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        """
        Args:
...
if __name__ == "__main__":
    ...
    dataset = HuBERTDataSet("/home/lqf/workspace/icassp2023/myhubert/exp/data/hubert_9", "IEMOCAP", "train")
    sampler = BucketizeBatchSampler(
        dataset.len_list,
        num_buckets=1000,
        max_token_count=87.5 * 16000,
        min_len=32000,
        max_len=250000,
        shuffle=False,
    )
    dataloader = DataLoader(
        ...
        collate_fn=CollateFnHubert(feature_type='hubert', pad=False, rand_crop=True),
        num_workers=10,
    )

    for batch in dataloader:
        waveforms, labels, audio_lengths, attention_mask = batch
