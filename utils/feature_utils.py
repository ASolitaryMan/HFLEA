...
def extract_feature_hubert(
    ...
    result = feature_extractor(waveform, sampling_rate=feature_extractor.sampling_rate, return_tensors="pt")
    model.eval()
    ...
    with torch.no_grad():
        feat = model.inference(input_values, attention_mask=attention_mask, output_hidden_states=True).hidden_states[layer_index]
...
def dump_features(
    ...
    pretrained_hubert_path: Optional[Path] = None,
    ...
    if feature_type == "hubert":
        ...
        config = AutoConfig.from_pretrained(pretrained_hubert_path)
        #if any question, may occur here.
        model = HubertModelPreTrainBase(config, num_cluster=num_cluster, feature_grad_mult=0.1).from_pretrained(checkpoint_path, num_cluster=num_cluster, feature_grad_mult=0.1)
        model.to(device)
        # model = _load_state(model, checkpoint_path, device)
        feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(pretrained_hubert_path, return_attention_mask=True)
    ...
    with open(tsv_file, "r") as f:
        ...
        for line in lines:
            ...
            if feature_type == "mfcc":
                ...
                feature = extract_feature_hubert(path, device, sample_rate, model, layer_index, feature_extractor)
            features.append(feature.cpu())
            lens.append(feature.shape[1])
